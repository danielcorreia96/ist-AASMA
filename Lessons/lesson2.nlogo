...
;;;
to robot-loop
  ifelse cell-has-box? and not box-cargo? and ramp-cell?
  [grab-box]
  [ifelse box-cargo? and shelf-cell? and not cell-has-box? and  (shelf-color? = cargo-box-color?)
    [drop-box]
    [ifelse free-cell?
      [move-ahead]
      [rotate]
...
;;;
to-report free-cell?
  report( [kind] of (patch-ahead 1) = ROOM_FLOOR and not any? robots-on (patch-ahead 1))
end


;;;
;;;  Check if the cell is a shelf
;;;
...
;;;
to-report wall-cell?
  report([kind] of (patch-ahead 1) = WALL)
end

;;;
;;;  Check if the cell has a box
;;;
